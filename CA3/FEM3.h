...
//#include <deal.II/grid/tria_boundary_lib.h> Deprecated Library in current versions
...
for(unsigned int globalDOFIndex=0; globalDOFIndex < totalDOFs; globalDOFIndex++){
	  if(dofLocation[globalDOFIndex][2] == 0.){
		boundary_values[globalDOFIndex] = 0.0;
	  }
  }
}
  ...
  for (;elem!=endc; ++elem){
	...
	for(unsigned int i=0; i<dim; i++){ //Loop over nodal dofs
	  for (unsigned int B=0; B<nodes_per_elem; B++) {
		...
		Klocal[dim*A+i][dim*B+k]+= C(i,j,k,l)*fe_values.shape_grad(dim*A+i,q)[j]*fe_values.shape_grad(dim*B+k,q)[l]*fe_values.JxW(q);	
		}
	      }
	    }
	  }
	...
	for (unsigned int q=0; q<num_face_quad_pts; ++q){
	  ...
	  for (unsigned int A=0; A<nodes_per_elem; A++){ //loop over all element nodes
		...
		For det(J) times the total quadrature weight: fe_face_values.JxW(q)*/
			Flocal[dim*A+i] += fe_face_values.shape_value(dim*A+i,q)*fe_face_values.JxW(q)*h[i];
...
void FEM<dim>::output_results (){
  ...
  std::ofstream output1 ("solution1.vtk");
